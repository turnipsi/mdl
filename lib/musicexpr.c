/* $Id: musicexpr.c,v 1.55 2016/01/11 21:15:40 je Exp $ */
...
static int
add_musicexpr_to_simultence(struct musicexpr_t *,
			    struct musicexpr_t *,
			    struct simultence_state *,
			    int);
...
musicexpr_to_midievents(struct musicexpr_t *me, int level)
{
	...
	simultence = musicexpr_to_simultence(me_workcopy, level + 2);
	if (simultence == NULL) {
			    ...
			    int level)
{
	...
	switch (me->me_type) {
		...
		if (offsetexpr == NULL) {
			warn("malloc in add_musicexpr_to_simultence");
...
musicexpr_to_simultence(struct musicexpr_t *me, int level)
{
	...
	if (add_musicexpr_to_simultence(simultence, me, &state, level) != 0) {
	...
	if (state.length > state.length_no_rests) {
		if ((rest = malloc(sizeof(struct musicexpr_t))) == NULL) {
			warn("malloc in musicexpr_to_simultence");
			musicexpr_free(simultence);
			return NULL;
		}
...
musicexpr_log(const struct musicexpr_t *me, int loglevel, int indentlevel)
{
	...
	case ME_TYPE_ABSNOTE:
		...
		break;
	case ME_TYPE_CHORD:
		mdl_log(loglevel, indentlevel, "%s\n", metype_string);
	...
	case ME_TYPE_RELNOTE:
		mdl_log(loglevel,
			indentlevel,
			"%s notesym=%d notemods=%d length=%.3f" \
			  " octavemods=%d\n",
			metype_string,
